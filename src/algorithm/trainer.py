...
class RepStateMachineTrainerRL:
    ...
    def train_ppo(self):
        ...
        while True:
            ...
            while not done:
                ...
                agent_selection_active, agent_selection_passive = self.agent.choose_action(state, num_locs)
    ...
    def collect_expert_knowledge(self, num_samples: int = 10_000):
        state, _ = self.env.reset()
        action = (self.env.inner_env.no_op_active(), self.env.inner_env.no_op_passive())
        state, reward, done, _, _ = self.env.step(action)
        done = False
        ...
        for i in tqdm(range(num_samples), desc="Collecting expert samples..."):
            action, reward = self.env.base_env.compute_best_next_action()
            next_state, reward, done, _, _ = self.env.step(action[0])
            self.expert_samples.append((state, action, reward, next_state, done))
            state = next_state

        logging.info(f"Collected {num_samples} expert samples")
        # pickle the expert samples
        with open("expert_samples.pkl", "wb") as f:
            pickle.dump(self.expert_samples, f)
...
def main():
    ...
    ppo_config = SwapPPOAgentConfigActionTypeBoth(swap_active, critic_active, None, None,
    ...
    training_args = TrainingParams(train_steps=2000)
...
def dqn():
    ...
    config = CustomNetworkConfig(num_centers=3, clusters=clusters, num_clients=50, render_mode="human",
                                 render_type="2d", device=device, penalty_weights=penalty_weights, num_active=3,
                                 display_all_latencies_from_start=True, action_type=ActionType.ACTIVE,
                                 )
    ...
    training_args = TrainingParams(train_steps=50, num_train_runs=1000000)
    ...
    trainer = RepStateMachineTrainerRL(trainer_config)
    trainer.collect_expert_knowledge(100)
    trainer.train()
