...
class SwapPPOAgentConfigActionTypeBoth:
    ...
    passive_actor: SwapGNN | TransformerSwapGNN | None
    passive_critic: CriticSwapGNN | TransformerGNN | None
...
class PPOAgentActionTypeBoth(AbstractAgent):
    ...
    def learn(self):
        ...
        for epoch in range(self.n_epochs):
            ...
            for batch in batch_iterator:
                ...
                objective_active_regularized = objective_active + self.entropy_reg * self._compute_entropy(
                    log_prob_new_active)
                ...
                if self.swap_actor_passive is not None:
                    ...
                    objective_passive_regularized = objective_passive + self.entropy_reg * self._compute_entropy(
                        log_prob_new_passive)
                    ...
                    full_loss = actor_active_loss + critic_active_loss
    ...
    def choose_action(self, state: torch_geometric.data.Batch | torch_geometric.data.Data, num_locs: int) \
        ...
        if self.swap_actor_passive is not None:
            ...
            remove_action = int(np.random.choice(torch.where(
                state.passive_mask[:num_locs] == -np.inf)[0].cpu()))
            add_action = int(np.random.choice(torch.where(
                state.passive_mask[:num_locs] != -np.inf)[0].cpu()))
            passive_action = (remove_action, add_action)
            passive_action = (torch.tensor(passive_action), torch.tensor(((1,1))), torch.tensor((10)).view(1,1))
