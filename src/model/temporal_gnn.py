...
class SemiTemporalEmbeddingConstructor(nn.Module):
    ...
    def forward(self, data_full: List[torch_geometric.data.Data]):
        ...
        # **What do we input in the LSTM?** We have (batch_size x num_locations x num_timesteps x embedding_dim) dimension
        # We merge the first two dimensions to get huge embedding that contains for each location its embedding over
        # the time steps. We do this over a whole batch, so in the batch we have a all node states over all the time
        # steps for each graph in the batch. This is then used for the LSTM and we model the time sequence of nodes
        # with the LSTM (time_horizon node instances per element in the huge batch).
